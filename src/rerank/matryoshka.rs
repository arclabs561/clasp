...
//! let refined = matryoshka::refine(&candidates, &query, &docs, 2).unwrap();
...
/// # Errors
///
/// Returns `RerankError::InvalidInput` if `head_dims >= query.len()`.
///
/// # Note
///
...
/// # Errors
///
/// Returns [`RerankError::InvalidHeadDims`] if `head_dims >= query.len()`.
...
mod tests {
    ...
    fn test_head_dims_too_large() {
        ...
        let query = vec![1.0, 2.0];
        let docs = vec![("d1", vec![1.0, 2.0])];
        // head_dims (2) >= query.len() (2) should return error
